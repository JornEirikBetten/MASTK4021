...
posterior = sstats.norm.pdf(x, loc = loc, scale = scale)
"""
...
phi_x = np.array([np.ones(1000), x-1900])
locs = mu.T @ phi_x 
fig = plt.figure()
...
plt.legend()
plt.savefig(fig_path + "scatter_predictive_mean.pdf", format = "pdf", bbox_inches = "tight")